...
impl Lexer {
	...
	fn tokenize_marching(&mut self, start_pos: &CursorPointing) -> bool {
		if let Char::Char(c) = self.current_char {
			match c {
				...
				// 处理符号
				'(' => self.add_token(TokenKind::Symbol(Symbol::LParen), false),
				')' => self.add_token(TokenKind::Symbol(Symbol::RParen), false),
				'{' => self.add_token(TokenKind::Symbol(Symbol::LBrace), false),
				'}' => self.add_token(TokenKind::Symbol(Symbol::RBrace), false),
				'[' => self.add_token(TokenKind::Symbol(Symbol::LBracket), false),
				']' => self.add_token(TokenKind::Symbol(Symbol::RBracket), false),
				';' => self.add_token(TokenKind::Symbol(Symbol::Semicolon), false),
	...
	fn scan_ident_or_keyword(&mut self) {
		...
		let kind = match ident.as_str() {
			...
			"true" => TokenKind::Value(Value::Bool(true)),
			"false" => TokenKind::Value(Value::Bool(false)),
			...
			"as" => TokenKind::Key(Key::ObjectAllocation(KeyObjectAllocation::As)),
			"var" => TokenKind::Key(Key::ObjectAllocation(KeyObjectAllocation::Var)),
			"val" => TokenKind::Key(Key::ObjectAllocation(KeyObjectAllocation::Val)),
	...
	fn scan_number(&mut self) {
		...
		let kind = if is_float {
			match f64::from_str(if radix != 10 { &num_str[2..] } else { &num_str }) {
				Ok(num) => TokenKind::Value(Value::NumFloat(num)),
				Err(_) => {
					let pos = self.cursor.get_pointing();
					self.errors.push((
					...
					TokenKind::Value(Value::NumFloat(0.0))
				}
			}
		} else {
			match i64::from_str_radix(if radix != 10 { &num_str[2..] } else { &num_str }, radix) {
				Ok(num) => TokenKind::Value(Value::NumInt(num)),
				Err(_) => {
					let pos = self.cursor.get_pointing();
					self.errors.push((
						LexError::MalformedNumber,
					...
					TokenKind::Value(Value::NumInt(0)) // 占位值
				}
			}
		};
	...
	fn scan_string(&mut self) {
		...
		self.tokens.push(Token {
			kind: TokenKind::Value(Value::String(string_vec)),
			span: Span {
				start_line: start_pos.num_line,
				start_col: start_pos.num_column,
				end_line: self.cursor.get_pointing().num_line,
	...
	fn scan_char(&mut self) {
		...
		self.tokens.push(Token {
			kind: TokenKind::Value(Value::Char(char_str.chars().next().unwrap_or('\0'))),
			span: Span {
				start_line: start_pos.num_line,
				start_col: start_pos.num_column,
				end_line: self.cursor.get_pointing().num_line,
			...
			if let Char::Char('{') = self.current_char {
				...
				while let Char::Char(c) = self.current_char {
					if c == '}' {
						self.next(); // 跳过 '}'
						// 添加Unicode转义标记
						return char::from_u32(u32::from_str_radix(&hex_str, 16).unwrap_or_else(
							|_| {
								let pos = self.cursor.get_pointing();
								self.errors.push((
	...
	fn scan_comment(&mut self, start_pos: &CursorPointing) -> bool {
		...
		match self.current_char {
			...
			Char::Char('=') => {
				self.tokens.push(Token {
					kind: TokenKind::Op(Op::SlashAssign),
					span: Span {
						start_line: start_pos.num_line,
						start_col: start_pos.num_column,
						end_line: self.cursor.get_pointing().num_line,
	...
	pub fn report(&self) {
		for (error, span) in &self.errors {
			...
			println!(
				...
				"^".repeat(span.end_col.saturating_sub(span.start_col) + 1)
					.bright_yellow()
			);
		}
