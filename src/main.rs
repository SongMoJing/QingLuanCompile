...
lazy_static::lazy_static! {
	...
	static ref HOST_STR: String = HOST.to_string();
}
...
fn main() {
    ...
    println!("输出的打包方式：{}", args.package.as_str());
    println!("编译时允许的最大线程数：{}", args.thread_count);
    // 查看projectPath是否存在
        ...
        Command::new(FILE_NAME.as_str())
            ...
            .arg(
                Arg::new("package")
                    .short('p')
                    .long("package")
                    .help(t!("ARGS.package", package = Package::Debug.as_str()).to_string())
                    .value_name("debug|release")
                    .value_parser(clap::value_parser!(Package))
                    .default_value(Package::Debug.as_str())
            ...
            .arg(
                Arg::new("os")
                    .short('o')
                    .long("os")
                    .help(t!("ARGS.os", os = HOST_STR.as_str()).to_string())
                    .value_name("os")
                    .default_value(HOST_STR.as_str())
                    .hide_possible_values(true)
                    .hide_default_value(true),
            )
            // 语言
            ...
            .arg(
                Arg::new("thread_count")
                    .short('t')
                    .long("thread_count")
                    .help(t!("ARGS.thread_count", thread_count = 4).to_string())
...
{{after-help}}"#,
    ...
    pub fn parse() -> Self {
        ...
        let matches = match cmd.try_get_matches() {
            Ok(matches) => matches,
            Err(err) => Log::new(LogType::Err, err.to_string().as_str()).throw(13),
        };
        ...
        let mut cli = Self {
            ...
            package: matches.get_one::<Package>("package").unwrap().clone(),
            os: matches.get_one::<String>("os").unwrap().to_string(),
            language: matches.get_one::<String>("language").unwrap().to_string(),
            thread_count: matches.get_one::<u8>("thread_count").unwrap().clone(),
        };
        ...
        if let Some(project_path) = matches
            ...
            .map(|s| s.to_string())
        {
            cli.project_path = project_path.replace("\\", "/");
        } else {
